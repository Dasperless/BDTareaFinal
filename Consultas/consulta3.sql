...
GO
CREATE OR ALTER PROCEDURE [dbo].[consulta3] --Nombre del procedimiento
	@inNombreDelGiro VARCHAR(50),   --Variables de entrada del SP
	@inAno INT,
	@OutIdConsulta3 INT OUTPUT,
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION TransCons3

		SELECT TOP (10) co2.Nombre
			,co.SumaPuntosMontaña
		...
		COMMIT TRANSACTION TransCons3; --Finaliza la transacción
	END TRY

	BEGIN CATCH
		IF @@TRANCOUNT > 0
			ROLLBACK TRANSACTION TransCons3;
